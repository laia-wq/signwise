import {normalizeName,missedLetters,PracticeSequence,SetupGate,framing} from './practice.js?v=7';
import {lessons} from './lessons.js?v=7';
import {assessHand,MotionTracker} from './coach.js?v=7';
import {HoldGate,CameraTest} from './session.js?v=7';
...
function processFrame(r,now){
 ...
 if(l.motion){
  ...
  $('hold-label').textContent=matched?'Movement matched':movement.phase==='shape'?'Pen off · show the starting handshape':movement.phase==='arming'?'Hold still · getting the pen ready':movement.phase==='retry'?'Stroke cleared · hold the starting shape again':`${digit} pen on · ${movement.score===0?'start your stroke':movement.score===35?'top stroke seen':movement.score===65?'finish the bottom stroke':'downstroke seen · finish the hook'}`;
  drawMotion(matched);
 }
