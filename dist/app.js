import {normalizeName,missedLetters,PracticeSequence,SetupGate,framing} from './practice.js?v=6';
import {lessons} from './lessons.js?v=6';
import {assessHand,MotionTracker} from './coach.js?v=6';
import {HoldGate,CameraTest} from './session.js?v=6';
...
function render(){const l=lessons[selected];$('lessons').innerHTML=lessons.map((x,i)=>`<button class="lesson-button ${i===selected?'active':''}" data-index="${i}" aria-pressed="${i===selected}" aria-label="${x.name}" ${test&&test.state!=='done'?'disabled':''}><span class="lesson-glyph">${x.id==='ILY'?'♡':x.id}</span><span><strong>${x.name}</strong><small>${x.sub}</small></span><span class="lesson-check">${completed.has(x.id)?'✓':''}</span></button>`).join('');$('title').textContent=test&&test.state!=='done'?`Make the letter ${l.id}`:l.id==='ILY'?'I love you':`Meet the letter ${l.id}`;$('letter').textContent=l.id;$('description').textContent=l.desc;$('steps').innerHTML=l.steps.map(s=>`<li>${s}</li>`).join('');$('tip').textContent=l.tip;$('hand-figure').hidden=!l.image;if(l.image){$('hand-reference').src=l.image;$('hand-reference').alt=`ASL ${l.id}${l.motion?' handshape and movement arrows':' handshape reference'}`;}$('motion-note').hidden=!l.motion;$('motion-note').textContent=`${l.id} includes movement. Follow the arrows; hold the starting shape, then trace the letter slowly.`;$('coach-mode').textContent=l.motion?'Camera motion matching':'Camera handshape matching';$('camera-prompt').textContent='Your turn to sign';$('camera-help').textContent='Enable your camera for a live match score.';$('image-source').href='https://commons.wikimedia.org/wiki/File:Asl_alphabet_gallaudet.svg';$('reference').textContent=l.id==='ILY'?'View ASL University reference ↗':'View HandSpeak alphabet reference ↗';$('reference').href=l.id==='ILY'?'https://www.lifeprint.com/asl101/topics/ily.htm':'https://www.handspeak.com/topic/408/';$('count').textContent=`${[...completed].filter(id=>id!=='ILY').length} / 26`;$('completion-status').textContent=completed.has(l.id)?'Completed by camera ✓':'Matches are completed automatically';$('next').innerHTML=selected===26?'Back to letter A ↻':'Next handshape →';$('image-error').hidden=true;renderTest();renderSequence();}
function selectLesson(index){if(sequence||test&&test.state!=='done')return;selected=index;resetTracking();render();feedback('Ready when you are.',lessons[index].motion?'Show the starting shape, then draw the movement slowly.':'Reach 88/100 with a clear, distinct match and hold for just over a second.');}
...
$('reset').onclick=()=>{sequence=null;missed=[];if(test)endTest();$('retry-actions').hidden=true;completed.clear();try{localStorage.removeItem('signwise-camera-completed-v1');localStorage.removeItem('signwise-camera-test-v1');}catch{}render();feedback('Camera progress reset.','New matches will be recorded automatically.');};
...
function processFrame(r,now){
 if(!setupReady){
  resetTracking();$('overlay-key').hidden=true;ctx.clearRect(0,0,canvas.width,canvas.height);
 ...
 if(sequence?.state==='release'){
  resetTracking();ctx.clearRect(0,0,canvas.width,canvas.height);$('overlay-key').hidden=true;
 ...
 if(sequence?.state==='done'){ctx.clearRect(0,0,canvas.width,canvas.height);$('overlay-key').hidden=true;return;}
 if(r.landmarks.length!==1){$('overlay-key').hidden=true;$('setup-message').textContent=r.landmarks.length>1?'Show one hand for reliable tracking.':'No hand tracked.';resetTracking();ctx.clearRect(0,0,canvas.width,canvas.height);feedback(r.landmarks.length>1?'Show only one hand.':'Bring your hand into view.',test?.state==='running'?'The timer is running.':'Keep all fingertips visible.');return;}
 const frame=framing(r.landmarks[0]);
 if(!frame.ok){resetTracking();ctx.clearRect(0,0,canvas.width,canvas.height);$('overlay-key').hidden=true;$('setup-message').textContent=frame.message;feedback('Tracking is unclear.',frame.message+(test?.state==='running'?' The timer is still running.':''));return;}
 ...
 if(l.motion){const m=motion.update(l.id,r.landmarks[0][l.id==='J'?20:8],now,result.valid&&result.match,handed,result.features?.screenPalm);score=Math.round(result.score*.45+m.score*.55);matched=m.match&&result.match;progress=m.score/100;$('hold-label').textContent=matched?'Movement matched':`Trace ${l.id} slowly · motion ${m.score}%`;}
 else{progress=hold.update(l.id,result.valid&&result.match,now);matched=progress>=1;$('hold-label').textContent=result.match?`Hold steady · ${Math.round(progress*100)}%`:result.score>=88?'Make the letter more distinct':'Reach a clear 88/100 match';}
 ...
 else feedback(result.title,l.motion?'Show the starting shape, then trace the movement. '+result.detail:result.detail);
}
...
function renderTest(){try{const saved=JSON.parse(localStorage.getItem('signwise-camera-test-v1')||'null');$('test-last').textContent=saved&&Number.isInteger(saved.correct)&&Number.isInteger(saved.total)?`Last completed test on this device: ${saved.correct} / ${saved.total}`:'';}catch{}const live=test&&test.state!=='done';$('test-banner').hidden=!live;document.body.classList.toggle('testing',!!live);document.body.classList.toggle('test-review',test?.state==='review');$('test-home').hidden=!!live;$('test-finish').hidden=test?.state!=='done';$('next').hidden=!!live;
