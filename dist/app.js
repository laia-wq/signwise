import {bestAlphabetResult,nextStepEligible} from './progress.js?v=14';
import {TrackingReport} from './diagnostics.js?v=14';
import {normalizeName,missedLetters,PracticeSequence,SetupGate,framing} from './practice.js?v=14';
import {lessons} from './lessons.js?v=14';
import {assessHand,MotionTracker} from './coach.js?v=14';
import {HoldGate,CameraTest} from './session.js?v=14';
...
function renderProgress(){
 ...
 $('best-context').textContent=bestResult?`${Math.round(bestResult.correct/26*100)}%${bestResult.seconds?` · ${bestResult.seconds}s per letter`:''} · ${Number.isFinite(Date.parse(bestResult.date))?new Date(bestResult.date).toLocaleDateString():'Saved on this device'}`:'Take the final test to set a score.';
...
function finishTest(){missed=missedLetters(test.results);$('retry-actions').hidden=!missed.length;$('retry-summary').textContent=`Review ${missed.length} missed ${missed.length===1?'letter':'letters'} with hints, then try again.`;clearInterval(testInterval);testInterval=null;document.body.classList.remove('testing','test-review');const total=test.total,n=test.ids.length;$('test-final-score').textContent=`${total} / ${n} · ${Math.round(total/n*100)}%`;$('test-results').replaceChildren();for(const r of test.results){const row=document.createElement('li');row.textContent=`${r.letter}: ${r.correct?'Correct':'Incorrect (timed out)'}`;$('test-results').append(row);}const result={correct:total,total:n,results:test.results,seconds:test.limit/1000,date:new Date().toISOString()};bestResult=bestAlphabetResult(bestResult,result);$('learning-next').hidden=!nextStepEligible(result);$('test-reflection').textContent=n===26?'A test checks recall today. Try again another day to check retention.':'This was a shorter test. Take all 26 letters to update your best alphabet score.';try{localStorage.setItem('signwise-camera-test-v1',JSON.stringify(result));if(bestResult)localStorage.setItem('signwise-best-alphabet-v1',JSON.stringify(bestResult));}catch{}render();feedback('Test complete.',`${total} of ${n} letters matched before the time limit.`,true);$('test-finish').scrollIntoView({behavior:'smooth',block:'center'});}
